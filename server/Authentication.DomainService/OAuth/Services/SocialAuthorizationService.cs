...
{
    public class SocialAuthorizationService : ITokenService
    {
        private readonly ILogger<SocialAuthorizationService> _logger;
        ...
        public async Task<TokenResponse> AuthenticateAsync(TokenRequest request, AuthenticationConfiguration authenticationConfiguration, User? user = null)
        {
            ...
            {
                return new TokenResponse { Error = "External provider did not provided any email or userPrincipalName", ErrorDescription = "External provider did not provided any email", StatusCode = 401 };
            }
