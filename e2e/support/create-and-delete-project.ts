...
/** Blocks console on Logic or OS — re-authenticates when the session expired. */
...
/** Logic project dashboard — Workflow sidebar link (fails fast if bounced to console). */
export async function waitForLogicDashboardReady(page: Page, projectName: string) {
  const ready = page.getByRole("link", { name: "Workflow" })
  ...
  if (outcome === "console" || (await bouncedToConsole())) {
    throw new Error(
      `Expected project dashboard for "${projectName}" but landed on the console. ` +
        ...
        "(project/environment localStorage). Re-run logic-setup.",
    )
  }
 ...
 *   navigate to Logic console → open the new project → Logic dashboard
 */
export async function createProject(page: Page) {
  ...
  await test.step("Return to Logic console", async () => {
  ...
  await test.step("Open the newly created project on Logic", async () => {
