...
test.describe("Authentication", () => {
  ...
  test("logs in through dev-iam and lands on the console", async ({ page }) => {
    ...
    // 1. Blocks Logic login page — a single CTA that starts the OIDC flow.
    ...
    // 3. Back on Blocks Logic, authenticated → console.
    ...
    // the installed kit defaults it to false, so the empty-state "Welcome to
    // SELISE Blocks" branch is unreachable here: the heading is always
