...
const LogDetails = ({ log }: { log: ProxyExecutionLog }) => (
  <div className="grid gap-3 border-t bg-muted/20 px-4 py-3 text-sm lg:grid-cols-2">
    ...
    <div>
      <span className="text-xs font-medium uppercase text-muted-foreground">Injected credentials</span>
...
export const ProxyLogsTab = ({ proxy, active }: { proxy: Proxy; active: boolean }) => {
  ...
  const { data = [], isFetching } = useGetProxyExecutions(proxy.id, filter, {
    live: active && live && proxy.enabled,
    enabled: active,
  });
  const { data: allRows = [] } = useGetProxyExecutions(proxy.id, "all", { enabled: active });
      ...
      <CardContent className="space-y-4 p-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="xs" className="gap-1.5" onClick={() => setLive((value) => !value)}>
