...
const validateExpression = (content: string, nodes: WorkflowNode[]): boolean => {
  ...
  const simpleMatch = content.match(/^json\.output(?:\.([a-zA-Z0-9_]+(?:\.[a-zA-Z0-9_]+)*))?$/);
  if (simpleMatch) {
    return true;
  }
  ...
  if (nodeInputMatch) {
    ...
    return nodes.some(n => n.name === nodeName || n.data?.name === nodeName || n.data?.label === nodeName);
  }

  // Check if it's the node format for json.output (optionally followed by parameters)
  const nodeMatch = content.match(/^node\["([^"]+)"\]\.json\.output(?:\.([a-zA-Z0-9_]+(?:\.[a-zA-Z0-9_]+)*))?$/);
  if (nodeMatch) {
...
export const ExpressionHighlighter = ({
  ...
  const enhancedChild = React.cloneElement(children, {
    ...
    className: `${children.props.className || ""} relative z-10 !bg-transparent !text-transparent caret-black dark:caret-white`.trim(),
  });
