...
export const refreshAccessToken = async (projectKey: string): Promise<string | null> => {
  try {
    ...
    const url = `${import.meta.env.BLOCKS_API_BASE_URL}${AUTH_ENDPOINTS.TOKEN}`;
...
export const getOidcCredential = async (
  ...
  try {
    const url = `${import.meta.env.BLOCKS_API_BASE_URL}${AUTH_OIDC_ENDPOINTS.GET_OIDC_CLIENT}?ProjectKey=${payload.projectKey}&ClientId=${payload.clientId}`;
...
export const userAcknowledgement = async (
  ...
  try {
    const url = `${import.meta.env.BLOCKS_API_BASE_URL}${OIDC_FLOW_ENDPOINTS.USER_ACKNOWLEDGEMENT}`;
...
export const accountRecover = async (
  ...
  try {
    const url = `${import.meta.env.BLOCKS_API_BASE_URL}${ACCOUNT_ENDPOINTS.RECOVER}`;
