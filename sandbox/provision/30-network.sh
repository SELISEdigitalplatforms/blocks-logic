...
RULES_CHANGED=no
write_file "$NFT_FILE" 0644 <<NFT && RULES_CHANGED=yes || true
#!/usr/sbin/nft -f
...
write_file "$APPLY_BIN" 0755 <<'APPLY' && RULES_CHANGED=yes || true
#!/usr/bin/env bash
...
write_file /etc/systemd/system/blocks-fn-firewall.service 0644 <<'UNIT' && RULES_CHANGED=yes || true
[Unit]
...
# The runner BindsTo this unit, so restarting it stops the runner. Reapply only when the
# policy changed, when the unit is not running, or when the table is missing from the
# kernel — which together is every case where a restart would have changed anything.
FW_ACTIVE=no; systemctl is-active --quiet blocks-fn-firewall.service && FW_ACTIVE=yes || true
FW_LOADED=no; /usr/sbin/nft list table inet blocks_fn >/dev/null 2>&1 && FW_LOADED=yes || true
if [ "${RULES_CHANGED:-yes}" = yes ] || [ "$FW_ACTIVE" = no ] || [ "$FW_LOADED" = no ]; then
  info "applying the firewall (rules changed: ${RULES_CHANGED:-yes}, unit active: $FW_ACTIVE, table loaded: $FW_LOADED)"
  ...
  info "firewall rules unchanged and the table is loaded - not restarting the unit"
fi
