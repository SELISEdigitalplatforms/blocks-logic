# Contributing to blocks-logic

Thank you for your interest in contributing to **blocks-logic**. Whether you are reporting a bug, suggesting an enhancement, or submitting code changes, your input is welcome.
...
Match the style already in the log. Most commits use [Conventional Commits](https://www.conventionalcommits.org/) (for example `test(client): cover workflow editor default edge and output panel branches`, `fix(sast): resolve SonarQube bugs and hotspots in domain services`); a plain imperative subject is also used for straightforward changes:

- Format: `type(scope): subject`, for example `feat(workflow): ...`, `fix(client): ...`, `docs: ...`
- Use the imperative mood and keep the subject concise; do not end it with a period
